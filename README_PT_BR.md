# Cyber-Labs-Writeups

[ ![English](https://img.shields.io/badge/Read_in-English-blue?style=flat-square) ](README.md)

---

Foco Principal: Segurança Ofensiva (Red Teaming) e Testes de Penetração.


## Estrutura do Repositório

Este repositório é organizado por ano e categoria para acompanhar minha evolução técnica:

### CTFs e Write-ups
Documentação detalhada e metodologia de resolução de desafios:
* [2025](./CTFs/2025/)
* [2026](./CTFs/2026/)

## Ferramentas e Payloads
  
### Conjunto de Habilidades Técnicas
- Scripts personalizados, ferramentas de automação e exploits desenvolvidos durante pesquisas laboratoriais.

| Categoria | Habilidades e Ferramentas |
| :--- | :--- |
| **Red Team** | Web Exploitation (LFI, IDOR, SQLi), Path Traversal, Reverse Shells, Escalada de Privilégio. |
| **Ferramentas (Ofensivas)** | **Burp Suite**, **Nmap**, **Gobuster**, **Metasploit**, **FFUF**, **Netcat**. |
| **Criptoanálise** | Quebra de cifra XOR, Ataques de Texto Claro Conhecido (Known Plaintext), Identificação de Hashes. |
| **Ferramentas (Crypto)** | **John the Ripper**, **Hashcat**, **CyberChef**, **Python (Scripts Customizados)**. |
| **Blue Team** | **Splunk (SIEM)**, Análise de Logs, Análise de Malware, Threat Hunting. |
| **Sistemas/Dev** | Linux/Unix Internals, Análise de arquivos Shadow, Automação em Python, PHP, Bash. |

---

## Informações de Contato

<p>
  <a href="https://www.linkedin.com/in/andrey-ms" target="_blank">
    <img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="mailto:andreym.professional@gmail.com">
    <img src="https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
  </a>
  <a href="https://medium.com/@andreymsdev" target="_blank">
    <img src="https://img.shields.io/badge/-Medium-%23000000?style=for-the-badge&logo=medium&logoColor=white" alt="Medium" />
  </a>
  <a href="https://tryhackme.com/p/Tux10" target="_blank">
    <img src="https://img.shields.io/badge/-TryHackMe-%23212C42?style=for-the-badge&logo=tryhackme&logoColor=white" alt="TryHackMe" />
  </a>
  <a href="https://devpost.com/andreymsdev" target="_blank">
    <img src="https://img.shields.io/badge/-Devpost-%23003E54?style=for-the-badge&logo=devpost&logoColor=white" alt="Devpost" />
  </a>
</p>

---
Aviso: Este repositório tem fins apenas educativos.
