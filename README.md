# Cyber-Labs-Writeups

---

Main Focus: Offensive Security (Red Teaming) and Penetration Testing.


## Repository Structure

This repository is organized by year and category to track technical progression:

### CTFs and Write-ups
Detailed walkthroughs and methodology for solving challenges:
* 2025
* 2026

## Tools and Payloads
  
### Technical Skill Set
- Custom scripts, automation tools, and exploits developed during laboratory research.

| Category | Skills and Tools |
| :--- | :--- |
| **Red Team** | Web Exploitation (LFI, IDOR, SQLi), Path Traversal, Reverse Shells, Privilege Escalation. |
| **Tools (Offensive)** | **Burp Suite**, **Nmap**, **Gobuster**, **Metasploit**, **FFUF**, **Netcat**. |
| **Cryptanalysis** | XOR Cipher Cracking, Known Plaintext Attacks, Hash Identification. |
| **Tools (Crypto)** | **John the Ripper**, **Hashcat**, **CyberChef**, **Python (Custom Scripts)**. |
| **Blue Team** | **Splunk (SIEM)**, Log Analysis, Malware Analysis, Threat Hunting. |
| **Systems/Dev** | Linux Internals, Shadow File Analysis, Python Automation, PHP, Bash. |

---

## Contact Information

<p>
  <a href="https://www.linkedin.com/in/andrey-ms" target="_blank">
    <img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="mailto:andreym.professional@gmail.com">
    <img src="https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
  </a>
  <a href="https://medium.com/@andreymsdev" target="_blank">
    <img src="https://img.shields.io/badge/-Medium-%23000000?style=for-the-badge&logo=medium&logoColor=white" alt="Medium" />
  </a>
  <a href="https://tryhackme.com/p/Tux10" target="_blank">
    <img src="https://img.shields.io/badge/-TryHackMe-%23212C42?style=for-the-badge&logo=tryhackme&logoColor=white" alt="TryHackMe" />
  </a>
    <a href="https://devpost.com/andreymsdev" target="_blank">
  <img src="https://img.shields.io/badge/-Devpost-%23003E54?style=for-the-badge&logo=devpost&logoColor=white" alt="Devpost" />
</a>
</p>


---
Disclaimer: This repository is for educational purposes only.
